/*
  ...
  Synopsis: Solution for Assignment 4 Question 1
 */
...
public class Animal
{
     ...
     * @param employeeID the staff member to remove
     */
    public void removeStaff(String employeeID)
    {
        if (!this.hasStaff(employeeID))
            throw new NoSuchElementException(employeeID + " is not assigned to this animal");

        Iterator<StaffMember> iter = assignedStaff.iterator();
        while (iter.hasNext()) {
            StaffMember stf = iter.next();
            if (stf.getEmployeeID().equals(employeeID)) {
                iter.remove();
            }
        }
    }
