...
public class PetStore
{
     ...
     * @param minKennelLabel the label of the first kennel in the store
     * @param maxKennelLabel the label of the last kennel in the store
     * @precond storeName != null && !storeName.equals("") && minKennelLabel >= 0 && maxKennelLabel >= minKennelLabel
     */
    public PetStore(String storeName, int minKennelLabel, int maxKennelLabel)
    {
        ...
        if (minKennelLabel < 0 || maxKennelLabel < minKennelLabel)
        {
            throw new IllegalArgumentException("The kennel labels " + minKennelLabel + " and " + maxKennelLabel
                    + " are invalid as they cannot be negative, and there must be at least one kennel between minimum and maximum values.");
        }

        name = storeName;
        this.minKennelLabel = minKennelLabel;
        manKennelLabel = maxKennelLabel;
        kennel = new Animal[maxKennelLabel - minKennelLabel + 1];
    }
    ...
    /**
     * @return a list of the (kennel labels of) empty kennels of the petstore
     */
    public ArrayList<Integer> availableKennels() {
        ArrayList<Integer> kennel_list = new ArrayList<>();
        for (int i = minKennelLabel; i <= manKennelLabel; i++)
            if (!isOccupied(i))
                kennel_list.add(i);
        return kennel_list;
    }
    ...
    /**
     * Removing the animal from a specific kennel.
     *
     * @param kennelLabel the label of the kennel from an animal
     * @precond isValidLabel(kennelLabel) && isOccupied(kennelLabel)
     */
    public void freeKennel(int kennelLabel) {
        if (!isValidLabel(kennelLabel))
            throw new IllegalArgumentException("The value " + kennelLabel + " is not a valid label");
     ...
     */
    private boolean isValidLabel(int kennelLabel)
    {
        return kennelLabel >= minKennelLabel && kennelLabel <= manKennelLabel;
    }
    ...
    public static void main(String[] args)
    {
        ...
        // testing for the available kennel method and freeKennel method.
        PetStore petstore = new PetStore("Bruce-Wayne", 15, 30);
        Animal p = new Animal("Andrew ", "A89", "Dog");
        Animal p1 = new Animal("Dan", "D78", "Cat" );

        petstore.assignAnimalToKennel(p, 15);
        petstore.assignAnimalToKennel(p1, 21);
        p = new Animal("Regina", "rhl156", "cat");
        p1 = new Animal("Tiffany", "tyg000", "dog");
        petstore.assignAnimalToKennel(p, 30);
        petstore.assignAnimalToKennel(p1, 27);

        ArrayList<Integer> available_kennels = new ArrayList<>();
        available_kennels.add(16);available_kennels.add(17); available_kennels.add(18);available_kennels.add(19);
        available_kennels.add(20);available_kennels.add(22);available_kennels.add(23);available_kennels.add(24);
        available_kennels.add(25);available_kennels.add(26);available_kennels.add(28);available_kennels.add(29);


        if(!petstore.availableKennels().equals(available_kennels)){
            System.out.println("availableKennel() failed, expected: " + available_kennels + " instead got: "+ petstore.availableKennels());
        }

        try{
            petstore.freeKennel(15);
        }
        catch (IllegalStateException e){
            if(!e.getMessage().equals("kennel 15 is currently not occupied by an animal")){
                System.out.println("The constructor threw an IllegalStateException, but not because the kennel label is already occupied");
            }
        }
